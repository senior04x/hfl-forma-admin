document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchMatches = async () => {
        try {
            ...
            console.error(err);
            loadingMatches.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: #ef4444;">O\'yinlarni yuklashda xatolik yuz berdi.</div>';
        }
    };
    ...
    const renderMatches = (filter) => {
        ...
        if (filtered.length === 0) {
            matchesGrid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: #94a3b8; padding: 40px;">Hech qanday o\'yin topilmadi.</div>';
            return;
        }
            ...
            const html = `
                <div class="match-card">
                    ...
                    <div class="match-details">
                        ...
                        ${match.youtube_link ? `
                        <div class="detail-row">
                            <i data-lucide="video" style="width: 14px; height: 14px; color: #ef4444;"></i> 
                            <a href="${match.youtube_link}" target="_blank" rel="noreferrer" class="youtube-link">Jonli ko'rish</a>
                        </div>
                        ` : ''}
                    </div>
                </div>
