-- Session-scoped team and roster browsing for captain transfers.
BEGIN;
CREATE INDEX IF NOT EXISTS applications_transfer_team_roster_idx
    ON public.applications(team_id,status,id) WHERE team_id IS NOT NULL;
...
BEGIN
    ...
    ELSIF p_action='teams' THEN
        SELECT coalesce(jsonb_agg(to_jsonb(r) ORDER BY r.name,r.id),'[]'::jsonb) INTO v_items FROM (
    ...
    RETURN jsonb_build_object('status',200,'items',
        CASE WHEN jsonb_array_length(v_items)>(CASE WHEN p_action='history' THEN 20 ELSE 30 END)
             THEN v_items-(CASE WHEN p_action='history' THEN 20 ELSE 30 END) ELSE v_items END,
        'next_cursor',CASE WHEN jsonb_array_length(v_items)>(CASE WHEN p_action='history' THEN 20 ELSE 30 END)
             THEN v_items->(CASE WHEN p_action='history' THEN 19 ELSE 29 END)->>'id' ELSE NULL END);
END;
$$;
